...
# 157 countries
countries = ['Brunei Darussalam', 'Comoros', 'Djibouti', 'Iraq', 'Libya', 'Serbia', 'Somalia', 'Suriname']

regions = df['region'].unique()

# selecting rows based on condition | ~ is not in
selected_df = df[~df['countries'].isin(countries)]
selected_df.to_csv('../../2024/selected_countries.csv', index=False)
...
def regions_analysis(indicators=main):
    ...
    for i in indicators:
        ...
        for j in selected_df['region'].unique():
            ...
            change = a[-1] - a[0]
            print('Change in {0} score in {1} (2000-2022): {2}'.format(j, i, round(change, 2)))

            plt.plot(range(2000, 2022 + 1), a, label=j)
            plt.legend(title='Regions', bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.title('{} Over Time (2000-2022)'.format(i))
            plt.xlabel('Year')
            plt.ylabel('Score')
...
# ----------------------------------- charts
# density_plot(2000, 2022, indicator='ef_score')

# improve_deteriorate(2000, 2022)

# top_bottom_10(2000, 2022)

# gap_10_percent(2000, 2022)
