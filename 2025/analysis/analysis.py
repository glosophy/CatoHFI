...
# read csv
df = pd.read_csv('../../2025/cleaning/hfi2025_cc.csv')
...
all_categories = ['hf_score', 'pf_rol', 'pf_ss', 'pf_movement', 'pf_religion', 'pf_assembly', 'pf_expression',
                  ...
                  'ef_legal-gender', 'ef_money', 'ef_trade', 'ef_regulation', 'ef_score']

# 158 countries
countries = ['Brunei Darussalam', 'Comoros', 'Djibouti', 'Libya', 'Somalia', 'Suriname', 'Timor-Leste']

regions = df['region'].unique()

# selecting rows based on condition | ~ is not in
selected_df = df[~df['countries'].isin(countries)]
selected_df.to_csv('../../2025/selected_countries.csv', index=False)
...
def improve_deteriorate(year1, year2, indicator='hf_score'):
    ...
    print('-' * 25)
    print('Change in {0} ({1}-{2}):'.format(indicator, year1, year2), round(diff_mean, 2))
    print('{0} {1}:'.format(indicator, year1), round(float(np.mean(np.array(selected_base))), 2))
    print('{0} {1}:'.format(indicator, year2), round(float(np.mean(np.array(selected_comparison))), 2))
    ...
    print('-' * 25)
    print('Change in {0} ({1}-{2}):'.format(indicator, year3, year2), round(all_change_hf, 2))
    print('{0} {1}:'.format(indicator, year3), round(float(np.mean(np.array(all_previous))), 2))
    print('{0} {1}:'.format(indicator, year2), round(float(np.mean(np.array(all_comparison))), 2))
    print('-' * 25)
...
def gap_10_percent(year1, year2, indicator='hf_score'):
    ...
    diff3_10 = highest10[9] - lowest10[9]

    print('Gap in {} between highest and lowest 10% (158 countries):'.format(indicator))
    print('Gap 2000:', round(diff1_10, 2))
    print('Gap 2010:', round(diff3_10, 2))
    print('Gap 2023:', round(diff2_10, 2))
...
def regions_analysis(indicators=main):
    """
    It plots a set of given indicators over time (2000-2022) for all regions.
    :param indicators: 'main' == hf_score, pf_score, ef_score. Can also pass a list with other indicators.
    ...
    for i in indicators:
        ...
        for j in selected_df['region'].unique():
            ...
            change = a[-1] - a[0]
            print('Change in {0} score in {1} (2000-2023): {2}'.format(j, i, round(change, 2)))

            plt.plot(range(2000, 2023 + 1), a, label=j)
            plt.legend(title='Regions', bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
            plt.title('{} Over Time (2000-2023)'.format(i))
            plt.xlabel('Year')
            plt.ylabel('Score')
            plt.tight_layout()
...
# ----------------------------------- charts
# density_plot(2000, 2023, indicator='ef_score')

# improve_deteriorate(2000, 2023)

# top_bottom_10(2000, 2022)

# gap_10_percent(2000, 2023)
